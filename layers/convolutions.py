...
class ConvBlock(nn.Module):
    ...
    """
    def \
            __init__(self, in_channels, out_channels=None, kernel_size=3, stride=1, norm='bn', activation='relu',
    ...
    """Residual block:
       x -> Conv -> norm -> act. -> Conv -> norm -> act. -> ADD -> out
         |                                                   |
          ---------------------------------------------------
    """
    def __init__(self, in_channels, out_channels=None, kernel_size=3, norm='bn', activation='relu', bias=False):
        super().__init__()
        out_channels = out_channels or in_channels

        self.layers = nn.Sequential(OrderedDict([
            ('conv_1', ConvBlock(in_channels, in_channels, kernel_size=kernel_size, stride=1, norm=norm, activation=activation, bias=bias)),
            ('conv_2', ConvBlock(in_channels, out_channels, kernel_size=kernel_size, stride=1, norm=norm, activation=activation, bias=bias)),
